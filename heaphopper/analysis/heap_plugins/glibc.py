...
class GlibcPlugin(AbstractHeapPlugin):
    ...
    def setup_project(self):
        ...
        # Find write_target
        self.write_target_var = self.proj.loader.main_object.get_symbol(
            'write_target')
    ...
    def setup_state(self):
        ...
        if self.config['use_vsa']:
            # vsa for satisfiability
            added_options.add(angr.options.APPROXIMATE_SATISFIABILITY)
            # vsa for guards
            added_options.add(angr.options.APPROXIMATE_GUARDS)
            # vsa for mem_sizes
            added_options.add(angr.options.APPROXIMATE_MEMORY_SIZES)
            # vsa for mem_indices
            added_options.add(angr.options.APPROXIMATE_MEMORY_INDICES)
        ...
        self.state = self.proj.factory.entry_state(
            add_options=added_options, remove_options=removed_options)
        ...
        self.state.memory.read_strategies = [
            angr.state_plugins.symbolic_memory.concretization_strategies.SimConcretizationStrategySolutions(
                16),
    ...
    def setup_vars(self):
        ...
        for i in range(20):
            cdata = self.proj.loader.main_object.get_symbol(
                'ctrl_data_{}'.format(i))
        ...
        mem2chunk_var = self.proj.loader.main_object.get_symbol(
            'mem2chunk_offset')
        self.var_dict['mem2chunk_addr'] = mem2chunk_var.rebased_addr
        mem2chunk = self.state.solver.BVV(
            value=self.config['mem2chunk_offset'], size=8 * 8)
        self.state.memory.store(
            self.var_dict['mem2chunk_addr'], mem2chunk, 8, endness='Iend_LE')
        ...
        if sdata_var:
            for i in range(0, self.config['sym_data_size'], 8):
                ...
                self.state.memory.store(
                    sdata_var.rebased_addr + i, smem_elem, 8, endness='Iend_LE')
                self.var_dict['sdata_addrs'].append(sdata_var.rebased_addr + i)

            # create entry in sym_data state storage
            self.var_dict['sym_data_ptr'] = sdata_var.rebased_addr + \
                self.config['mem2chunk_offset']
        ...
        for i in range(0, self.write_target_var.size, 8):
            ...
            self.state.memory.store(
                self.write_target_var.rebased_addr + i, write_mem_elem, 8, endness='Iend_LE')
            self.var_dict['wtarget_addrs'].append(
                self.write_target_var.rebased_addr + i)
            self.var_dict['global_vars'].append(
                self.write_target_var.rebased_addr + i)

        # Set header size
        header_size_var = self.proj.loader.main_object.get_symbol(
            'header_size')
        self.var_dict['header_size_addr'] = header_size_var.rebased_addr
        header_size = self.state.solver.BVV(
            value=self.config['header_size'], size=8 * 8)
        self.state.memory.store(
            self.var_dict['header_size_addr'], header_size, 8, endness='Iend_LE')

        # Set malloc sizes
        malloc_size_var = self.proj.loader.main_object.get_symbol(
            'malloc_sizes')
        self.var_dict['malloc_size_addrs'] = [
            malloc_size_var.rebased_addr + i for i in range(0, malloc_size_var.size, 8)]
        ...
        if self.config['chunk_fill_size'] == 'zero':
            self.var_dict['fill_size_vars'] = [self.state.solver.BVV(
                0, 8 * 8)] * len(self.var_dict['fill_size_addrs'])
        if self.config['chunk_fill_size'] == 'header_size':
            self.var_dict['fill_size_vars'] = [header_size] * \
                len(self.var_dict['fill_size_addrs'])
        ...
        if type(self.config['chunk_fill_size']) in (int, int):
            self.var_dict['fill_size_vars'] = [claripy.BVV(
                self.config['chunk_fill_size'], 8 * 8)] * len(self.var_dict['fill_size_addrs'])

        for fsize, fill_var in zip(self.var_dict['fill_size_addrs'], self.var_dict['fill_size_vars']):
            # zero-fill first just in case
            self.state.memory.store(
                fsize, claripy.BVV(0, 8 * 8), endness='Iend_LE')
            self.state.memory.store(fsize, fill_var, endness='Iend_LE')

        # Set overflow sizes
        overflow_size_var = self.proj.loader.main_object.get_symbol(
            'overflow_sizes')
        overflow_size_offset = overflow_size_var.rebased_addr
        self.var_dict['overflow_sizes_addrs'] = [
        ...
        for i in range(0, arb_write_var.size, 8):
            self.var_dict['arb_offset_vars'].append(
                arb_write_var.rebased_addr + i)
        ...
        # get winning addr
        self.var_dict['winning_addr'] = self.proj.loader.main_object.get_symbol(
    ...
    def process_state(self, num_results, state, write_state, fd):
        ...
        input_opts = state.solver.eval_upto(
            all_bytes(state.posix.fd[fd].read_storage), num_results, cast_to=bytes)
        for input_opt in input_opts:
            s = state.copy()
            write_s = write_state.copy()
            s.add_constraints(
                all_bytes(s.posix.fd[fd].read_storage) == input_opt)
            write_s.add_constraints(
                all_bytes(write_s.posix.fd[fd].read_storage) == input_opt)
            ...
            if 'sym_data_ptr' in self.var_dict and s.heaphopper.sym_data_states[self.var_dict['sym_data_ptr']]:
                ...
                sym_s.add_constraints(
                    all_bytes(sym_s.posix.fd[fd].read_storage) == input_opt)
                if stdin_opt:
            ...
            header_var = s.memory.load(
                self.var_dict['header_size_addr'], 8, endness="Iend_LE")
            ...
            for write_target in self.var_dict['wtarget_addrs']:
                wt_var = write_s.memory.load(
                    write_target, 8, endness="Iend_LE")
            ...
            if s.heaphopper.arb_write_info:
                arb_write = {k: s.solver.eval(v) for k, v in list(
                    s.heaphopper.arb_write_info.items())}
