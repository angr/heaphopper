...
def bin_addr2src_line(bin_addr, bin_file):
    with open(bin_file, 'rb') as bf:
        ...
        _, line = decode_file_line(dwarfinfo, bin_addr)
        return line
...
def trace(config_name, binary_name):
    ...
    if config['use_mem_limiter']:
        sm.use_technique(MemLimiter(
            config['mem_limit'], config['drop_errored']))
    if config['use_dfs']:
        sm.use_technique(angr.exploration_techniques.DFS())
    if config['use_veritesting']:
        sm.use_technique(angr.exploration_techniques.Veritesting())
    if config['use_concretizer']:
        concr_addrs = heap_plugin.var_dict['malloc_size_addrs'] + \
            heap_plugin.var_dict['allocs']
    ...
    sm.use_technique(angr.exploration_techniques.Explorer(
        find=heap_plugin.finds, avoid=heap_plugin.avoids))
    ...
    real_fd = heap_plugin.state.posix.open(
        path, flags=Flags.O_RDONLY, preferred_fd=mem_corr_fd)
    ...
    if config['input_pre_constraint']:
        if 'overflow' in config['zoo_actions']:
            overflow_bytes = config['zoo_actions']['overflow'] * \
                max(config['overflow_sizes'])
        else:
            overflow_bytes = 0
        ...
        if 'arb_relative_write' in config['zoo_actions']:
            arw_bytes = config['zoo_actions']['arb_relative'] * \
                heap_plugin.state.arch.byte_width * 2
        else:
            arw_bytes = 0

        num_bytes = overflow_bytes + uaf_bytes + arw_bytes
        input_bytes = heap_plugin.state.posix.fd[mem_corr_fd].read_data(num_bytes)[
            0].chop(8)
    ...
    sm.move(from_stash='found', to_stash='vuln',
    ...
    for path in found_paths:
        last_instr_addr = get_last_instr(
            heap_plugin.proj, path.history.bbl_addrs.hardcopy)
    ...
    logger.info('Found %d vulns', len(found_paths))
    ...
    if len(found_paths) > 0:
        ...
        if config['store_desc']:
            store_vuln_descs(binary_name, found_paths,
                             heap_plugin.var_dict, arb_writes)
    # IPython.embed()
    return 0
...
def store_vuln_descs(desc_file, states, var_dict, arb_writes):
    ...
    logger.info('Creating vuln descriptions: %s-desc.yaml', desc_file)
    ...
    for state_num, state in enumerate(states):
        ...
        for msize, minfo in zip(var_dict['malloc_size_addrs'], bin_info['allocs']):
            size_val = state.solver.eval(
                state.memory.load(msize, 8, endness='Iend_LE'))
            desc.append(
                '\t\t* {} byte allocation to {}'.format(size_val, minfo[0]))
        # show frees
        ...
        for of, of_size in zip(bin_info['overflows'], var_dict['overflow_sizes_addrs']):
            size_val = state.solver.min(
                state.memory.load(of_size, 8, endness='Iend_LE'))
            desc.append(
                '\t\t* {} byte overflow from {} into {}'.format(size_val, of[0], of[1]))
        # show bad frees
        ...
        if state.heaphopper.double_free:
            desc.append(
                '\t- {} double free(s)'.format(len(state.heaphopper.double_free)))
        # show arbitrary relative writes
        desc.append(
            '\t- {} arb_relative_writes:'.format(len(bin_info['arb_relative_writes'])))
        for dst, offset in bin_info['arb_relative_writes']:
            desc.append(
                '\t\t* arbitrary relative write to {} at offset {}'.format(dst, offset))
        # show single bitflips
        desc.append(
            '\t- {} single_bitflips:'.format(len(bin_info['single_bitflips'])))
        for dst, offset, bit in bin_info['single_bitflips']:
            desc.append(
                '\t\t* single bitflip to {} at offset {} on bit {}'.format(dst, offset, bit))
        # show single bitflips
        desc.append('\t- {} uafs:'.format(len(bin_info['uafs'])))
        for dst, size in bin_info['uafs']:
            desc.append(
                '\t\t* use-after-free of {} with size of {}'.format(dst, size))
            ...
            for j in range(0, sol, 8):
                ...
                for input_var in input_vars:
                    if input_var in constraint_vars:
                        desc.append(
                            '\t\t* 8 byte of controlled data in the heap @ {}+{}'.format(read[0], i))
        ...
        for sdata in var_dict['sdata_addrs']:
            ...
            for mem_var in mem_vars:
                if mem_var in constraint_vars:
                    desc.append(
                        '\t\t* 8 byte of symbolic data in the bss @ {}'.format(sdata))
        ...
        if state.heaphopper.vuln_type == 'malloc_allocated':
            desc.append(
                '\t- malloc returns a pointer to an already allocated heap region')
...
def store_results(heap_plugin, num_results, bin_file, states, fd):
    logger.info('Storing result infos to: %s-result.yaml', bin_file)
    results = []
    arbitrary_writes = []
    for i, state in enumerate(states):
        result = heap_plugin.process_state(
            num_results, state, state.heaphopper.vuln_state, fd)
        result['path_id'] = i
